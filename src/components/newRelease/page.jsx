...
const imgUrl = (path, size = "w1280") => path ? `https://image.tmdb.org/t/p/${size}${path}` : "/placeholder.png";
...
export default async function NewRelease() {
  ...
  const title = hero.title || hero.name || hero.original_title || hero.original_name || "Untitled";
  ...
  return (
    <>
      <div className="relative h-[260px] md:h-[360px] lg:h-[420px] rounded-3xl overflow-hidden mx-4 md:mx-0">
        <Image src={imgUrl(bg, "w1280")} alt={title} fill className="object-cover" priority/>
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/30 to-black/0">
          <div className="absolute inset-0 p-6 md:p-8 flex items-center">
            <div className="max-w-xl">
              ...
              {subline && ( <div className="mt-2 text-sm md:text-base text-white/80">{subline}</div> )}
              <div className="mt-5">
                <Link href={href} className="px-5 py-2.5 rounded-full bg-[#023C26] text-[#CBF273] font-semibold shadow-[0_10px_30px_rgba(0,0,0,0.25)] hover:bg-[#CBF273] hover:text-[#131313] transition duration-500 ease-in-out">
                  Watch
                </Link>
              </div>
            </div>
